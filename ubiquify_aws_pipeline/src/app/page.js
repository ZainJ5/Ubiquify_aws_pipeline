...
// Mirrors the AWS IAM account password policy so failures surface before apply
  ...
  async function submit(e) {
    ...
    for (const u of users) {
      if (u.passwordMode === "custom") {
        const issues = passwordIssues(u.password);
        if (issues.length > 0) {
          setSubmitError(`Password for "${u.name || "unnamed user"}" needs ${issues.join(", ")}.`);
          return;
        }
      }
    }
      ...
      <form onSubmit={submit} className="mt-8 space-y-8">
        ...
        <section className="rounded-xl border border-black/10 dark:border-white/15 p-5">
                  ...
                  {user.passwordMode === "custom" && (
                    <div className="flex w-full flex-col">
                      <input
                        ...
                        placeholder="Aa1!... min 8"
                        minLength={8}
                        required
                        className={`w-full rounded-lg border bg-transparent px-3 py-2 text-sm ${
                          user.password && passwordIssues(user.password).length > 0
                            ? "border-red-500/60"
                      ...
                      {user.password && passwordIssues(user.password).length > 0 && (
                        <span className="mt-1 text-xs text-red-500">
                          Needs {passwordIssues(user.password).join(", ")}
                        </span>
                      )}
                    </div>
                  )}
          ...
          <p className="mt-3 text-xs opacity-60">
            ...
            passwords must be 8+ characters with an uppercase letter, lowercase letter, number,
            and symbol (AWS password policy).
          </p>
        </section>
                      ...
                      <tr key={u.name} className="border-b border-black/5 dark:border-white/10">
                        ...
                        <td className="py-2 font-mono">
                          {u.passwordMode === "custom"
                            ...
                            : u.password ?? "—"}
                          {u.passwordError && (
                            <div className="mt-1 font-sans text-xs text-amber-500">
                              Custom password rejected by AWS ({u.passwordError}) — the temporary
                              password above is active instead.
                            </div>
                          )}
                        </td>
                      </tr>
